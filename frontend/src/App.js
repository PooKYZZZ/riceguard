...
function App() {
  ...
  function submitSignup(e) {
    e.preventDefault();
    // Simple validation: email format and matching passwords
    setSignupError('');
    const email = signupEmail.trim();
    // Basic email regex (sufficient for client-side validation)
  ...
  useEffect(() => {
    ...
    function onFocusIn(e) {
      // Log the active element's tag, id, classes to the console
        ...
        <div className="modal-backdrop">
          <div className="modal" onClick={(e) => e.stopPropagation()}>
              <div className="modal-avatar-wrap">
                <img src={`${process.env.PUBLIC_URL}/user.png`} alt="user avatar" className="modal-avatar" />
              </div>
              <h2>Log in</h2>
        ...
        <div className="modal-backdrop">
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={submitSignup}>
              ...
              <div className="field">
                <span className="label-text">Password</span>
                <input id="signupPass" type="password" value={signupPass} onChange={(e) => setSignupPass(e.target.value)} required />
              </div>
              <div className="field">
                <span className="label-text">Confirm Password</span>
                <input id="signupConfirm" type="password" value={signupConfirm} onChange={(e) => setSignupConfirm(e.target.value)} required />
              </div>
              {signupError && (
                <div className="signup-error" role="alert" style={{ color: '#b91c1c', marginTop: '0.25rem' }}>{signupError}</div>
              )}
              <div className="modal-actions">
